...
void inserir(tipoPriorityQueue* fila, int valor) {
    ...
    while (i > 0 && fila->dados[i] < fila->dados[(i - 1) / 2]) { // Enquanto o valor inserido for menor que o pai, ele sobe no heap
        int tmp = fila->dados[i];
        fila->dados[i] = fila->dados[(i - 1) / 2]; // Acha o pai
...
// Implementação do HeapSort mínimo
void heapSort(tipoPriorityQueue* fila) {
    int tam = fila->tamanho;
    int* ordenada = (int*) malloc(tam * sizeof(int));

    int i = 0;
    while (i < tam) {
        ordenada[i] = remover(fila);
        i++;
    }
    ...
    while(j < tam) {
        printf("%d ", ordenada[j]);
        inserir(fila, ordenada[j]); // Reinsere para manter a fila
        j++;
    }
    printf("\n");
    free(ordenada);
